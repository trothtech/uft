...
/*      (void) sprintm(temp,"uft","srv",522,'E',0,NULL); */
    ...
    if (n < 0)
      {
        /*  "no spoolids available"  (or permission?)  */
    ...
    /*  all okay thus far,  send the herald (informative)  */
 ...
 */
        ...
        if (*q != 0x00) *q++ = 0x00;
        p = line;
        /*  q now points to args (if any), q to verb  */
        ...
        if (abbrev("USER",p,1))
          {
            ...
            *p = 0x00;
            ...
            if (n < 0)
              {
(void) sprintf(temp,"527 user slot error UID=%d EUID=%d ERRNO=%d",
...
#ifndef         UFT_ANONYMOUS
            (void) sprintf(temp,"118 SEQ=%s (user)",seqs);
            (void) uftdstat(1,temp);
#endif
        ...
        if (abbrev("DATA",p,3))
          {
            ...
            i = atoi(q);
            (void) sprintf(temp,"113 %d; send %d bytes of data.",i,i);
            (void) uftdstat(1,temp);
